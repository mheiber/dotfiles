...
command!      -bang -nargs=* Rip                               call fzf#vim#grep("rg --column --line-number --no-heading --color=always --smart-case -- ".shellescape(<q-args>), fzf#vim#with_preview(), <bang>0)

"nnoremap <leader>- i-<space>[ ]<space><Esc> 
inoremap <leader>- - [ ] 
nnoremap <leader>-i - [ ] 
...
" customization: 
function! ProgRock()
    let padding = 0 " set to something higher if you want more room in the status bar
    let width = winwidth(0)
    let lcur = line('.')
    let lend = line('$')
    let full = max([width - padding, 3])
    let cur_render = float2nr((lcur / (lend + 0.0)) * full)
    let rem_render = (full - cur_render) - 1
    return RepeatStr('=', cur_render) . RepeatStr(' ', rem_render). (padding == 0 ? '' : '|')
endfunction
...
" ripgrep smart case
" let g:rg_command = "rg --vimgrep -S -g '*.md'"
let g:rg_command = "rg --vimgrep -S -g '!*.pdf'"
...
nnoremap <LocalLeader>n  :MerlinGrowEnclosing<Cr>
